...
public class VisualizerFrame extends JFrame {
    ...
    private final String[] Sorts = {"Bubble", "Quick", "Selection", "Insertion", "Merge"};
    ...
    public VisualizerFrame() {
        ...
        arrayWrapper.setBackground(backgroundColor);
        buttonWrapper.setBackground(backgroundColor);

        start.setBackground(backgroundColor);
        start.setForeground(textColor);

        selection.setBackground(backgroundColor);
        selection.setForeground(textColor);

        speed.setBackground(backgroundColor);
        speed.setForeground(textColor);

        size.setBackground(backgroundColor);
        size.setForeground(textColor);

        speedVal.setForeground(textColor);
        sizeVal.setForeground(textColor);

        stepped.setBackground(backgroundColor);
        stepped.setForeground(textColor);
        ...
        addComponentListener(new ComponentListener() {
            ...
            public void componentResized(ComponentEvent e) {
                ...
                sizeModifier = (int) ((getHeight() * 0.9) / (barPanels.length));
            }
    ...
    // BarPanel class to represent each bar with a value
    private class BarPanel extends JPanel {
        private int value;

        public BarPanel(int value) {
            this.value = value;
        ...
        protected void paintComponent(Graphics g) {
            ...
            g.drawString(String.valueOf(value), getWidth() / 2 - 10, getHeight() / 2);
        }
    }
    ...
    public void preDrawArray(Integer[] squares) {
        barPanels = new BarPanel[SortingVisualizer.sortDataCount];
        arrayWrapper.removeAll();
        // 90% of the windows height, divided by the size of the sorted array.
        sizeModifier = (int) ((getHeight() * 0.9) / (barPanels.length));
        for (int i = 0; i < SortingVisualizer.sortDataCount; i++) {
            barPanels[i] = new BarPanel(squares[i]);
            barPanels[i].setPreferredSize(new Dimension(SortingVisualizer.blockWidth, squares[i] * sizeModifier));
            arrayWrapper.add(barPanels[i], c);
        }
        repaint();
        validate();
    }
    ...
    public void reDrawArray(Integer[] squares, int working, int comparing, int reading) {
        ...
        for (int i = 0; i < barPanels.length; i++) {
            ...
            barPanels[i].setPreferredSize(new Dimension(SortingVisualizer.blockWidth, squares[i] * sizeModifier));
            if (i == working) {
                barPanels[i].setBackground(Color.green);
            } else if (i == comparing) {
                barPanels[i].setBackground(Color.red);
            } else if (i == reading) {
                barPanels[i].setBackground(Color.yellow);
            } else {
                barPanels[i].setBackground(Color.blue);
            }
            arrayWrapper.add(barPanels[i], c);
        }
        repaint();
